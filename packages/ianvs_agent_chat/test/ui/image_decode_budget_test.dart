...
void main() {
  group('BoundedImagePreview', () {
    ...
    for (final stage in _DecodeStage.values) {
      testWidgets(
        ...
        (tester) async {
          ...
          final reservation = _CountingImageReservation();
          ChatImageDecodeCancellation? observedCancellation;
    ...
    testWidgets('replacing an installed image releases the previous owners', (
      ...
      Widget preview({
        ...
        required ChatImageDecodeReservation reservation,
      }) => MaterialApp(
        home: BoundedImagePreview(
          data: data,
          decoder: decoder,
    ...
    testWidgets('cancels a queued waiter and closes a late reservation once', (
      ...
      final reservationCompleter = Completer<ChatImageDecodeReservation>();
      final reservation = _CountingImageReservation();
      ChatImageDecodeCancellation? observedCancellation;
  ...
  group('ChatImageDecodeCancellationSource', () {
    test(
      'notifies each current listener once and is safe after cancellation',
      () {
        final source = ChatImageDecodeCancellationSource();
        var retainedCalls = 0;
        var removedCalls = 0;
  ...
  group('ChatImageDecodeBudgetLedger', () {
    test(
      'accepts exact global pixels and rejects the first pixel overage',
      () async {
        final exactLedger = ChatImageDecodeBudgetLedger(
          budget: _budget(
        ...
        final exactFirst = await exactLedger.acquire(
          decodedBytes: 1,
          cancellation: ChatImageDecodeCancellationSource(),
        );
        ...
        final overLedger = ChatImageDecodeBudgetLedger(
          budget: _budget(
            maxConcurrentImageDecodes: 3,
            maxImagePreviewPixelsGlobal: 20,
            maxImageDecodeBytesGlobal: 200,
    ...
    test(
      ...
      () async {
        final ledger = ChatImageDecodeBudgetLedger(
          budget: _budget(
        ...
        await expectLater(
          ...
          throwsA(isA<ChatInputLimitExceeded>()),
        );
    ...
    test('cancelling a waiter releases its provisional reservation', () async {
      ...
      final cancelledExpectation = expectLater(
        cancelledFuture,
        throwsA(isA<ChatImageDecodeCancelled>()),
      );
    ...
    test(
      ...
      () async {
        ...
        late Future<ChatImageDecodeReservation> failed;
    ...
    test(
      ...
      () async {
        ...
        final cancelled = ChatImageDecodeCancellationSource()..cancel();
    ...
    test(
      ...
      () async {
        ...
        await expectLater(
          ...
          throwsA(
            isA<ChatInputLimitExceeded>().having(
              (error) => error.observedAtLeast,
              'observedAtLeast',
              15,
            ),
...
final class _CountingImageReservation implements ChatImageDecodeReservation {
...
void _close(ChatImageDecodeReservation reservation) {
  reservation.finishDecode();
  reservation.releaseInstalledMemory();
}

final class _CountingCancellation implements ChatImageDecodeCancellation {
...
final class _HostileCancellation implements ChatImageDecodeCancellation {
