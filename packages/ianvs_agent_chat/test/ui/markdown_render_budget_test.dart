...
import 'package:ianvs_agent_chat/ui/markdown_render_budget.dart';
...
void main() {
    ...
    test('counts each supported syntax marker at exact and plus one', () {
      ...
      for (final testCase in cases) {
        ...
        final exact = scanMarkdownForRendering(
          source,
          budget: ChatInputBudget(maxMarkdownSyntaxTokens: exactTokenCount),
        );
        final plusOne = scanMarkdownForRendering(
          source,
          budget: ChatInputBudget(maxMarkdownSyntaxTokens: exactTokenCount - 1),
        );
        ...
        expect(plusOne.omission?.reason, ChatInputOmissionReason.inputLimit);
  ...
  group('writeBoundedMetadataPreview', () {
    test('honors Unicode scalar and UTF-8 byte exact boundaries', () {
      ...
      expect(charPlusOne.omission?.reason, ChatInputOmissionReason.inputLimit);
      expect(byteExact.text, '"éé"');
      expect(byteExact.omission, isNull);
      expect(bytePlusOne.omission?.reason, ChatInputOmissionReason.inputLimit);
    });
    ...
    test('rejects cycles and non-string keys without leaking values', () {
      ...
      for (final value in <Object?>[
        ...
        expect(
          ...
          ChatInputOmissionReason.invalidStructure,
        );
