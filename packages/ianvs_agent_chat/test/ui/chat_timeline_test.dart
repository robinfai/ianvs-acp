...
void main() {
  ...
  testWidgets('ChatTimeline collapses enhanced turn process behind summary', (
    ...
    final controller = _TestMessages();
    addTearDown(controller.dispose);
    controller.addMessageForTesting(
      ChatMessageData(role: ChatMessageRole.user, text: 'Implement the queue'),
      startsNewTurn: true,
    );
    controller.addMessageForTesting(
      ChatMessageData(
  ...
  testWidgets(
    ...
    (tester) async {
      await tester.pumpWidget(
        timeline([
          ChatMessageData(role: ChatMessageRole.user, text: 'First request'),
          ChatMessageData(
            role: ChatMessageRole.assistant,
          ...
          ChatMessageData(role: ChatMessageRole.user, text: 'Second request'),
          ChatMessageData(
            role: ChatMessageRole.assistant,
...
Review the screenshot''',
      ...
      controller.addMessageForTesting(
        ChatMessageData(role: ChatMessageRole.assistant, text: 'Reviewed.'),
      );
      controller.addMessageForTesting(
        ChatMessageData(role: ChatMessageRole.user, text: 'Apply the changes'),
        startsNewTurn: true,
      );
      controller.addMessageForTesting(
        ChatMessageData(role: ChatMessageRole.assistant, text: 'Applied.'),
      );
  ...
  testWidgets('ChatTimeline delegates image blocks without decoding in build', (
    ...
    final ledger = ChatImageDecodeBudgetLedger(budget: budget);
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      await expectLater(
        appOverlay,
        matchesGoldenFile('../goldens/file-diff-redesign-default.png'),
      );
      ...
      await expectLater(
        appOverlay,
        matchesGoldenFile('../goldens/file-diff-redesign-hover.png'),
      );
    },
  ...
  testWidgets('ChatTimeline never builds MarkdownBody after syntax overflow', (
    ...
    await tester.pumpWidget(
      timeline([
        ChatMessageData(role: ChatMessageRole.assistant, text: '**😀😀**'),
      ], inputBudget: budget),
    );
  ...
  testWidgets('ChatTimeline keeps MarkdownBody at the exact syntax limit', (
    ...
    const budget = ChatInputBudget(maxMarkdownSyntaxTokens: 4);
    await tester.pumpWidget(
      timeline([
        ChatMessageData(role: ChatMessageRole.assistant, text: '**safe**'),
      ], inputBudget: budget),
    );
...
```''',
      ...
      await expectLater(
        ...
        matchesGoldenFile('../goldens/markdown-rendering-polish.png'),
      );
    },
    skip: !Platform.isMacOS,
  );
  ...
  testWidgets('ChatTimeline displays one typed notice per omission kind', (
    ...
    const budget = ChatInputBudget(maxMarkdownSyntaxTokens: 1);
    final omission = ChatInputOmission(
  ...
  testWidgets('ChatTimeline caches expanded metadata by real revisions', (
    ...
    final message = _TestChatMessageData(
      role: ChatMessageRole.tool,
      text: 'exec_command',
      metadata: metadata,
    );
  ...
  testWidgets('ChatTimeline renders user and assistant messages', (
    ...
    await tester.pumpWidget(
      timeline([
        ChatMessageData(role: ChatMessageRole.user, text: 'Hello'),
        ChatMessageData(role: ChatMessageRole.assistant, text: 'Hello, human.'),
      ]),
    );
  ...
  testWidgets('ChatTimeline keeps scroll pinned when metadata changes', (
    ...
    final messages = List<ChatMessageView>.generate(
      18,
  ...
  testWidgets('ChatTimeline renders error card', (tester) async {
    await tester.pumpWidget(
      timeline([ChatMessageData(role: ChatMessageRole.error, text: 'boom')]),
    );
  ...
  testWidgets('ChatTimeline tool group has default hover and expanded states', (
    ...
    await tester.pumpWidget(
      timeline([
        ...
        ChatMessageData(role: ChatMessageRole.assistant, text: 'Done.'),
      ]),
    );
  ...
  testWidgets('ChatTimeline folds adjacent thinking into the tool group', (
    ...
    addTearDown(controller.dispose);
    controller.addMessageForTesting(
      ChatMessageData(role: ChatMessageRole.user, text: 'Check the timeline'),
      startsNewTurn: true,
    );
    controller.addMessageForTesting(
      ChatMessageData(
  ...
  testWidgets('ChatTimeline keeps assistant commentary outside tool groups', (
    ...
    addTearDown(controller.dispose);
    controller.addMessageForTesting(
      ChatMessageData(role: ChatMessageRole.user, text: 'Inspect the timeline'),
      startsNewTurn: true,
    );
    controller.addMessageForTesting(
      ChatMessageData(
  ...
  testWidgets('ChatTimeline preserves soft line breaks in user prompts', (
    ...
    await tester.pumpWidget(
      timeline([
        ChatMessageData(role: ChatMessageRole.user, text: 'hover\n展开\n默认'),
      ]),
    );
...
[@codex-clipboard-reference.png](file:///tmp/reference.png)[@image](/tmp/reference.png)''',
  ...
  testWidgets('ChatTimeline preserves tool disclosure state across streaming', (
    ...
    addTearDown(controller.dispose);
    controller.addMessageForTesting(
      ChatMessageData(role: ChatMessageRole.user, text: 'Run checks'),
      startsNewTurn: true,
    );
    controller.addMessageForTesting(
      ChatMessageData(
...
final class _TestMessages {
  final List<ChatMessageView> messages = <ChatMessageView>[];
  int _nextTurnId = 0;
  int? _activeTurnId;

  void addMessageForTesting(
    ChatMessageData message, {
    bool startsNewTurn = false,
  }) {
    if (startsNewTurn) {
...
final class _TestChatMessageData implements ChatMessageView {
  _TestChatMessageData({
    required this.role,
    required this.text,
  ...
  @override
  List<ChatInputOmission> omissions = const <ChatInputOmission>[];
