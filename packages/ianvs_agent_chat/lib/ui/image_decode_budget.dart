...
abstract interface class ChatImageDecodeReservation {
...
abstract interface class ChatImageDecodeCancellation {
...
final class ChatImageDecodeCancellationSource
    implements ChatImageDecodeCancellation {
  ...
  void cancel() {
    ...
    _listeners.clear();
    final errors = <_ChatImageDecodeCallbackError>[];
    for (final listener in listeners) {
      try {
        listener();
      } catch (error, stackTrace) {
        errors.add(_ChatImageDecodeCallbackError(error, stackTrace));
      }
    }
    for (final error in errors) {
      _reportChatImageDecodeCallbackError(error.error, error.stackTrace);
    }
  }
...
final class ChatImageDecodeCancelled implements Exception {
  const ChatImageDecodeCancelled();

  @override
  String toString() => 'ChatImageDecodeCancelled';
}

final class _ChatImageDecodeCallbackError {
  const _ChatImageDecodeCallbackError(this.error, this.stackTrace);

  final Object error;
  final StackTrace stackTrace;
}

void _reportChatImageDecodeCallbackError(Object error, StackTrace stackTrace) {
  try {
...
final class ChatImageDecodeBudgetLedger {
  ChatImageDecodeBudgetLedger({required ChatInputBudget budget})
    : _budget = budget..validate();

  final ChatInputBudget _budget;
  final LinkedList<_ChatImageDecodeWaiter> _waiters =
      LinkedList<_ChatImageDecodeWaiter>();

  var _activeDecodes = 0;
  var _reservedPreviewPixels = 0;
  var _reservedBytes = 0;

  Future<ChatImageDecodeReservation> acquire({
    required int decodedBytes,
    required ChatImageDecodeCancellation cancellation,
  }) {
    final bool isCancelled;
    try {
      isCancelled = cancellation.isCancelled;
    } catch (error, stackTrace) {
      return Future<ChatImageDecodeReservation>.error(error, stackTrace);
    }
    if (isCancelled) {
      return Future<ChatImageDecodeReservation>.error(
        const ChatImageDecodeCancelled(),
      );
    }
    if (decodedBytes < 0) {
    ...
    if (_activeDecodes < _budget.maxConcurrentImageDecodes) {
      _activeDecodes += 1;
      return Future<ChatImageDecodeReservation>.value(
        _ChatImageDecodeReservation(
          this,
          decodedBytes,
          previewPixels,
          reservedBytes,
    ...
    final waiter = _ChatImageDecodeWaiter(
    ...
    try {
      ...
      _grantWaiters();
      if (removeError != null) {
        _reportChatImageDecodeCallbackError(
          removeError.error,
    ...
    waiter.registrationInProgress = false;
    final completer = Completer<ChatImageDecodeReservation>();
    waiter.completer = completer;
    if (waiter.cancelledDuringRegistration) {
      completer.completeError(const ChatImageDecodeCancelled());
    } else {
      _grantWaiters();
    }
  ...
  void _cancelWaiter(_ChatImageDecodeWaiter waiter) {
    ...
    if (waiter.registrationInProgress) {
      ...
      waiter.completer?.completeError(const ChatImageDecodeCancelled());
    }
    _grantWaiters();
    if (removeError != null) {
      _reportChatImageDecodeCallbackError(
  ...
  _ChatImageDecodeCallbackError? _removeWaiterListener(
    _ChatImageDecodeWaiter waiter,
  ) {
    ...
    try {
      ...
      return _ChatImageDecodeCallbackError(error, stackTrace);
    }
    return null;
  }
...
final class _ChatImageDecodeWaiter
    extends LinkedListEntry<_ChatImageDecodeWaiter> {
  _ChatImageDecodeWaiter({
  ...
  final ChatImageDecodeCancellation cancellation;
  Completer<ChatImageDecodeReservation>? completer;
...
final class _ChatImageDecodeReservation implements ChatImageDecodeReservation {
  _ChatImageDecodeReservation(
    this._ledger,
    this._decodedBytes,
    this._previewPixels,
  ...
  final ChatImageDecodeBudgetLedger _ledger;
