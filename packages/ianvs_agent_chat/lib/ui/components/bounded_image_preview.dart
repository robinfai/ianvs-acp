...
typedef BoundedImageReservationAcquirer =
    Future<ChatImageDecodeReservation> Function({
...
class _BoundedImagePreviewState extends State<BoundedImagePreview> {
  var _generation = 0;
  ChatImageDecodeCancellationSource? _cancellation;
  ui.Image? _installedImage;
  ChatImageDecodeReservation? _installedReservation;
  ...
  void _startDecode() {
    ...
    final cancellation = ChatImageDecodeCancellationSource();
  ...
  bool _isCurrent(
    int generation,
    ChatImageDecodeCancellationSource cancellation,
  ) =>
      mounted &&
  ...
  Future<void> _decode(
    ...
    ChatImageDecodeReservation? reservation;
    BoundedImageBuffer? buffer;
    BoundedImageDescriptor? descriptor;
...
bool _validSourceDimensions(int width, int height, ChatInputBudget budget) {
