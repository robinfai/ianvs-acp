...
final class ChatBase64ScanResult {
  const ChatBase64ScanResult(this.encodedLength, this.decodedBytes);
...
final class ChatInputOmission {
  factory ChatInputOmission({
    required ChatInputOmissionReason reason,
    required String resource,
    ...
    if (reason == ChatInputOmissionReason.inputLimit && !hasCapacity) {
      throw ArgumentError(
  ...
  final ChatInputOmissionReason reason;
  ...
  Map<String, Object?> toJson() {
    ...
    if (reason == ChatInputOmissionReason.inputLimit) {
      json['limit'] = limit;
  ...
  String toString() {
    ...
    return 'ChatInputOmission(${fields.join(', ')})';
  }
}

final class ChatTextBudgetChunk {
  const ChatTextBudgetChunk({
...
final class ChatUtf8LineBudgetCheckpoint {
  ChatUtf8LineBudgetCheckpoint._({
...
final class ChatUtf8LineBudgetCounter {
  ...
  final List<ChatUtf8LineBudgetCheckpoint> _activeCheckpoints =
  ...
  void _requireActiveCheckpoint(ChatUtf8LineBudgetCheckpoint checkpoint) {
  ...
  ChatTextBudgetChunk _consume(String chunk, {required bool finish}) {
    final safePrefix = StringBuffer();
    var acceptedBytes = 0;
    ChatInputOmission? omission;
    var index = 0;
  ...
  ChatInputOmission _makeOmission(int limit, int observedAtLeast) =>
      ChatInputOmission(
  ...
  ChatTextBudgetChunk _emptyChunk() => ChatTextBudgetChunk(
    safePrefix: '',
    acceptedBytes: 0,
    totalBytes: _totalBytes,
    totalLines: _totalLines,
...
class ChatInputBudget {
  ...
  void validate() {
    ...
    chatMaxBase64EncodedLength(maxEmbeddedMediaBytes);
...
final class ChatStructuredUpdateGuard {
  ChatStructuredUpdateGuard({
    required ChatInputBudget budget,
    required String resource,
  }) : _budget = _validatedChatInputBudget(budget),
       _resource = resource;

  final ChatInputBudget _budget;
  final String _resource;
  ...
  Object? _copyJsonValue(
    ...
    void recordString(String string) {
      final count = _boundedChatUtf8Length(
        string,
        limit: _budget.maxStructuredStringBytes,
        onExceeded: (observedAtLeast) => _throwLimit(
          field: field,
  ...
  void _rejectReentrantMetadataAccess() {
    if (_metadataCopyInProgress) {
      throw StateError(
        'Cannot access an ChatStructuredUpdateGuard during a metadata copy.',
      );
    }
  }
...
final class ChatRetainedSizeEstimator {
  ChatRetainedSizeEstimator({required ChatInputBudget budget})
    : _budget = budget {
    budget.validate();
  }
  ...
  int estimate(Object? value) {
    ...
    while (pending.isNotEmpty) {
      ...
      if (current is ChatInputOmission) {
        checkDepth(valueFrame.depth);
        final isInputLimit =
            current.reason == ChatInputOmissionReason.inputLimit;
...
String _omissionReasonWireName(ChatInputOmissionReason reason) =>
    switch (reason) {
      ChatInputOmissionReason.inputLimit => 'input_limit',
      ChatInputOmissionReason.invalidEncoding => 'invalid_encoding',
      ChatInputOmissionReason.invalidImage => 'invalid_image',
      ChatInputOmissionReason.invalidStructure => 'invalid_structure',
    };

/// Capacity failure that never includes the rejected ACP payload.
class ChatInputLimitExceeded implements Exception {
  const ChatInputLimitExceeded({
    required this.resource,
    required this.limit,
    required this.observedAtLeast,
  });
  ...
  @override
  String toString() =>
      'ChatInputLimitExceeded(resource: $resource, limit: $limit, '
      'observedAtLeast: $observedAtLeast)';
}
...
copyBoundedInitializeInput({
  ...
  Object? agentInfo,
  ChatInputBudget budget = const ChatInputBudget(),
}) {
  budget.validate();
  ...
  final guard = ChatJsonInputGuard(
...
ChatInputBudget _validatedChatInputBudget(ChatInputBudget budget) {
  budget.validate();
  return budget;
}
