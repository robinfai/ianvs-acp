...
enum ChatPermissionDecision { allow, deny, cancel }

enum ChatPermissionInvalidationReason {
  timedOut,
  promptEnded,
  promptCancelled,
  sessionClosed,
...
final class ChatPermissionInvalidation {
  const ChatPermissionInvalidation({
  ...
  final ChatPermissionInvalidationReason reason;
  final DateTime invalidatedAt;
}

enum ChatPermissionAuditStatus { pending, allowed, denied, cancelled }

enum ChatPermissionDecisionSource {
  manual,
  trustRule,
  reviewAgent,
  policy,
...
enum ChatToolCallExecutionPolicy { defaultPermissions, autoReview, fullAccess }

class ChatPermissionChoice {
  const ChatPermissionChoice({
  ...
  ChatPermissionDecision? get explicitDecision {
    final normalized = kind?.trim().toLowerCase() ?? '';
    if (normalized == 'allow' ||
        normalized == 'allow_once' ||
        normalized == 'allow_always') {
      return ChatPermissionDecision.allow;
    }
    if (normalized == 'deny' ||
      ...
      return ChatPermissionDecision.deny;
    }
    return null;
  }

  ChatPermissionDecision? get decision {
    ...
    if (looksDenied) return ChatPermissionDecision.deny;
...
class ChatPermissionTrustRule {
  const ChatPermissionTrustRule({
  ...
  bool matches(ChatPermissionRequest request) {
  ...
  String get displayDecision {
    return switch (decision) {
      ChatPermissionDecision.allow => 'Allow',
      ChatPermissionDecision.deny => 'Deny',
      ChatPermissionDecision.cancel => 'Cancel',
    };
  }
}

class ChatPermissionRequest {
  const ChatPermissionRequest({
    ...
    this.choices = const <ChatPermissionChoice>[],
    this.toolKind,
  ...
  const ChatPermissionRequest._retained({
    required this.id,
    required this.lifecycleId,
    required this.title,
    required this.rationale,
  ...
  final List<ChatPermissionChoice> choices;
  ...
  ChatPermissionRequest withGeneration(int value) {
    ...
    final snapshot = ChatPermissionRequest._retained(
      id: id,
    ...
    return ChatPermissionRequest._retained(
      id: id,
      lifecycleId: lifecycleId,
      title: title,
      rationale: rationale,
  ...
  ChatPermissionRequest forAudit({
    ...
    final sourceSnapshot = ChatPermissionRequest._retained(
      id: id,
      lifecycleId: lifecycleId,
      title: this.title,
  ...
  ChatPermissionChoice? choiceById(String optionId) {
...
_freezePermissionRequestCollections({
  ...
  required List<ChatPermissionChoice> choices,
  required Map<String, Object?> metadata,
  required Map<String, Object?> transientPolicyContext,
}) {
  final guard = acp.ChatStructuredUpdateGuard(
    budget: const acp.ChatInputBudget(),
  ...
  List<ChatPermissionChoice> frozenChoices() {
    try {
      ...
      return List<ChatPermissionChoice>.unmodifiable(
        copied.map((value) {
          final choice = value! as Map<String, Object?>;
          return ChatPermissionChoice(
      ...
      return const <ChatPermissionChoice>[];
    }
  }
...
class ChatPermissionReviewResult {
  const ChatPermissionReviewResult({
    this.decision,
    this.risk = '',
    this.rationale = '',
    this.reviewer = '',
...
ChatPermissionReviewResult sanitizeChatPermissionReviewResult(
  ChatPermissionReviewResult result, {
  acp.ChatInputBudget inputBudget = const acp.ChatInputBudget(),
  ...
  try {
    ...
    return ChatPermissionReviewResult(
...
ChatPermissionReviewResult _permissionReviewSizeOmission() {
  return const ChatPermissionReviewResult(
    risk: 'unknown',
...
class ChatPermissionAuditEntry {
  const ChatPermissionAuditEntry({
  ...
  final ChatPermissionAuditStatus status;
  final DateTime recordedAt;
  final DateTime? resolvedAt;
  final ChatPermissionDecisionSource? decisionSource;
  final ChatPermissionReviewResult? reviewResult;
  final String? selectedOptionId;

  ChatPermissionAuditEntry copyWith({
    ChatPermissionRequest? request,
    ChatPermissionAuditStatus? status,
    DateTime? recordedAt,
    DateTime? resolvedAt,
    ChatPermissionDecisionSource? decisionSource,
    ChatPermissionReviewResult? reviewResult,
    String? selectedOptionId,
  }) {
    return ChatPermissionAuditEntry(
  ...
  String get displayStatus {
    return switch (status) {
      ChatPermissionAuditStatus.pending => 'Pending',
      ChatPermissionAuditStatus.allowed => 'Allowed',
      ChatPermissionAuditStatus.denied => 'Denied',
      ChatPermissionAuditStatus.cancelled => 'Cancelled',
    };
  }

  String? get displayDecisionSource {
    return switch (decisionSource) {
      ChatPermissionDecisionSource.manual => 'Manual',
      ChatPermissionDecisionSource.trustRule => 'Trust rule',
      ChatPermissionDecisionSource.reviewAgent => 'Review agent',
      ChatPermissionDecisionSource.policy => 'Policy',
      ChatPermissionDecisionSource.system => 'System',
      null => null,
    };
  }
...
int chatPermissionAuditEntryEncodedBytes(ChatPermissionAuditEntry entry) {
  return _encodedJsonBytes(entry.toJson());
}

Map<String, Object?>? _permissionAuditEntryForExport(
  ChatPermissionAuditEntry entry,
) {
  try {
    ...
    final source = <String, Object?>{
      ...
      if (reviewResult != null)
        'review': sanitizeChatPermissionReviewResult(reviewResult).toJson(),
    ...
    final copiedChoices =
        (copiedRequest['choices'] as List<Object?>? ?? const <Object?>[])
            ...
            .toList(growable: false);
    final boundedRequest = ChatPermissionRequest._retained(
      ...
      choices: List<ChatPermissionChoice>.unmodifiable(copiedChoices),
...
String chatPermissionAuditEntriesToJson(
  List<ChatPermissionAuditEntry> entries, {
