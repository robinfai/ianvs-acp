...
void main() {
  ...
  test(
    ...
    () async {
      ...
      expect(capabilities['filesystemReadTextFile'], isFalse);
      expect(capabilities['filesystemWriteTextFile'], isFalse);
  ...
  test(
    'process loss is visible and disposal closes the shared runtime',
    () async {
      final runtime = IanvsRustRuntime(
        native: FfiIanvsAcpNativeApi.open(libraryPath: libraryPath),
        pollInterval: const Duration(milliseconds: 1),
      ...
      runtime.startAgent(
        agentName: 'disconnect-fixture',
        command: agentPath,
        processCwd: root,
        environment: const {'IANVS_FIXTURE_EXIT_AFTER_NEW_SESSION': '1'},
      );
      await _nextWhere(iterator, (event) => event.status == 'ready');
      runtime.createSession(requestId: 'disconnect-session', cwd: root);
      await _nextWhere(
        iterator,
        (event) => event.update?['kind'] == 'session_created',
      );
      final disconnected = await _nextWhere(
        iterator,
        (event) => event.status == 'recovering',
      );
      expect(disconnected.data['detail'], contains('Agent exited'));
      // A paused StreamIterator must be cancelled before closing its stream.
      await iterator.cancel();
      await runtime.dispose();
      await runtime.dispose();
      expect(
        () => runtime.listSessions(requestId: 'after-dispose'),
        throwsStateError,
      );
      expect(await runtime.events.isEmpty, isTrue);
    },
    skip: artifactsAvailable
        ? false
        : 'Run tool/verify.sh to build native test artifacts.',
    timeout: const Timeout(Duration(seconds: 20)),
  );

  test(
    'terminal native launch failure reaches Dart as an error event',
    () async {
      final runtime = IanvsRustRuntime(
        native: FfiIanvsAcpNativeApi.open(libraryPath: libraryPath),
        pollInterval: const Duration(milliseconds: 1),
      ...
      final absent = Directory.systemTemp.createTempSync(
        'ianvs-missing-agent-',
      );
      addTearDown(() => absent.deleteSync());
      runtime.startAgent(agentName: 'missing', command: '${absent.path}/agent');
      final failure = await _nextWhere(
        iterator,
        (event) => event.type == IanvsRuntimeEventType.runtimeError,
      );
      expect(failure.errorCode, 'agent_runtime_failed');
      ...
      while (await iterator.moveNext()) {
        ...
        switch (event.type) {
          ...
          case IanvsRuntimeEventType.terminalExited:
            expect(
              (event.data['exit'] as Map)['exitCode'],
              0,
              reason: '${event.data}',
            );
            exited = true;
