...
class SessionManager implements AcpBoundedObservationSource {
  ...
  SessionManager({
    ...
    _elicitationCreateHandler = config.elicitationProvider;
    _mcpConnectHandler = config.mcpConnectProvider;
    _mcpMessageHandler = config.mcpMessageProvider;
    _mcpDisconnectHandler = config.mcpDisconnectProvider;
    ...
    peer.onElicitationCreate = _elicitationCreateHandler;
    peer.onMcpConnect = _mcpConnectHandler;
    peer.onMcpMessage = _mcpMessageHandler;
    peer.onMcpDisconnect = _mcpDisconnectHandler;
  }
  ...
  late final AcpRequestHandler? _elicitationCreateHandler;
  late final AcpRequestHandler? _mcpConnectHandler;
  late final AcpRequestHandler? _mcpMessageHandler;
  late final AcpRequestHandler? _mcpDisconnectHandler;
  ...
  void _detachOwnedPeerHandlers() {
    ...
    if (identical(peer.onElicitationCreate, _elicitationCreateHandler)) {
      peer.onElicitationCreate = null;
    }
    if (identical(peer.onMcpConnect, _mcpConnectHandler)) {
      peer.onMcpConnect = null;
    }
    if (identical(peer.onMcpMessage, _mcpMessageHandler)) {
      peer.onMcpMessage = null;
    }
    if (identical(peer.onMcpDisconnect, _mcpDisconnectHandler)) {
      peer.onMcpDisconnect = null;
    }
  }
  ...
  Future<InitializeResult> initialize({
    ...
    final copiedMeta = resp['_meta'] == null
        ? null
        : AcpJsonInputGuard(
            resource: 'ACP initialize metadata',
            maxDepth: inputBudget.maxCapabilityDepth,
  ...
  Future<void> deleteSession({required String sessionId}) =>
      _removeRemoteSession(sessionId, method: 'session/delete');

  /// Close an active session without deleting persisted history.
  Future<void> closeSession({required String sessionId}) =>
      _removeRemoteSession(sessionId, method: 'session/close');

  Future<void> _removeRemoteSession(
    String sessionId, {
    ...
    try {
      try {
        ...
        await peer.sendRaw(method, <String, dynamic>{'sessionId': sessionId});
  ...
  Future<void> _deliverTypedPromptWinner(
    ...
    try {
      ...
      if (winner.kind == JsonRpcPromptTerminalKind.response) {
        final terminal = _turnEndedFromPromptResponse(winner.response!);
  ...
  TurnEnded _turnEndedFromPromptResponse(Map<String, dynamic> response) {
    final guard = AcpJsonInputGuard(
      resource: 'ACP prompt terminal response',
      maxDepth: inputBudget.maxJsonDepth,
      maxNodes: inputBudget.maxCapabilityNodes,
      maxBytes: inputBudget.maxCapabilityBytes,
    );
    final copied = guard.copyMap(<String, Object?>{
      if (response['usage'] != null) 'usage': response['usage'],
      if (response['_meta'] != null) '_meta': response['_meta'],
    });
    final usage = copied['usage'];
    final meta = copied['_meta'];
    return TurnEnded(
      stopReasonFromWire((response['stopReason'] as String?) ?? 'other'),
      usage: usage is Map<String, dynamic> ? PromptUsage.fromJson(usage) : null,
  ...
  AcpUpdate _parseSessionUpdate({
    ...
    if (kind == 'user_message_chunk' ||
      ...
      final List<Object?> blocks = rawContent is List
          ? rawContent
          : <Object?>[rawContent];
      final parsed = MessageDelta.fromRaw(
    ...
    if (kind == 'config_option_update') {
      try {
        ...
        final parsed = ConfigOptionUpdate.fromJson(
          Map<String, dynamic>.unmodifiable(copied! as Map),
        );
        return ConfigOptionUpdate(
          List<ConfigOption>.unmodifiable(parsed.configOptions),
          meta: parsed.meta == null
              ? null
              : Map<String, dynamic>.unmodifiable(parsed.meta!),
        );
      } on AcpInputLimitExceeded catch (error) {
    ...
    if (kind == 'session_info_update') {
      try {
        ...
        final parsed = SessionInfoUpdate.fromJson(
          Map<String, dynamic>.unmodifiable(copied! as Map),
        );
    ...
    if (kind == 'plan_removed') {
      ...
      return PlanRemovedUpdate(boundedPlanId, meta: meta);
    }
  ...
  ({Object? value, int detachedBytes}) _retainedUpdateProjection(
    ...
    if (update is MessageDelta) {
      ...
      return (
        value: <String, Object?>{
          ...
          'content': contents,
          'isThought': update.isThought,
          if (update.messageId != null) 'messageId': update.messageId,
    ...
    if (update is PlanUpdate) {
      ...
      value = update.toJson();
    } else if (update is ConfigOptionUpdate) {
      value = <String, Object?>{
    ...
    } else if (update is SessionInfoUpdate) {
      value = <String, Object?>{
    ...
    } else if (update is StructuredPlanUpdate) {
      value = update.raw;
    } else if (update is PlanRemovedUpdate) {
      value = <String, Object?>{
  ...
  Future<Json> _onWriteTextFile(Json req, InboundAdmission rawAdmission) async {
    final admission = _permissionAdmissionForHandler(req, rawAdmission);
    try {
      if (config.fsProvider == null) {
        throw Exception('File system operations not supported');
  ...
  Json _unavailableTerminalWait() => <String, dynamic>{'exitCode': 0};
