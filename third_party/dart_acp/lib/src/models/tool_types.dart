...
class ToolCallLocation {
  ...
  factory ToolCallLocation.fromJson(
    ...
    return ToolCallLocation(
      path: path,
      line: line,
      meta: _boundedOptionalToolMetadata(
        json,
        guard,
        field: 'tool location metadata',
      ),
    );
  }
...
class ToolCall {
  ...
  static ToolCall _fromOwnedJson(
    ...
    final meta = _boundedOptionalToolMetadata(
      json,
      guard,
      field: 'tool metadata',
    );
  ...
  ToolCall _mergeOwned(
    ...
    final nextMeta =
        _boundedOptionalToolMetadata(update, guard, field: 'tool metadata') ??
        meta;
...
Map<String, dynamic>? _boundedOptionalToolMetadata(
  Map source,
  AcpStructuredUpdateGuard guard, {
  required String field,
}) {
  final value = _firstToolValue(source, const <String>['_meta']);
  if (identical(value, _absentToolField)) return null;
  if (value is! Map) {
    throw const FormatException('Invalid ACP tool metadata.');
  }
