  ...
  AgentEvent? _eventFromAcpUpdate(
    ...
    switch (update) {
      ...
      case acp.ConfigOptionUpdate():
        ...
        final options = updateSessionId == null
            ? _configOptionsFromAcpAtomically(update.configOptions) ??
                  const <AcpConfigOption>[]
            : _configOptionsBySession[updateSessionId] ??
                  const <AcpConfigOption>[];
        return AgentEvent(
          type: AgentEventType.status,
  ...
  void _cacheBoundedUpdate(String sessionId, acp.AcpUpdate update) {
    switch (update) {
      case acp.ModeUpdate():
        ...
        return;
      case acp.ConfigOptionUpdate():
        final mapped = _configOptionsFromAcpAtomically(update.configOptions);
        if (mapped == null) {
          _rejectConfigUpdate(sessionId, _invalidOmission('config_options'));
          return;
        }
    ...
    switch (block) {
      ...
      case acp.ResourceContent():
        final resource = <String, Object?>{
          'uri': block.uri,
          if (!block.embedded && block.title != null) 'title': block.title,
