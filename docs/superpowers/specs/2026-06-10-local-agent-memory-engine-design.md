...
当前项目是 Flutter macOS Desktop ACP 客户端，生产 ACP 会话由 Rust Core 通过 FFI
统一管理。`memory-core` 是独立的本地事实源，不代理 ACP。新的目标是在本地增加一个
可审计、默认关闭、按审批策略处理写入的长期记忆系统，用于五类 memory：
...
- `task_episode`：经审核的任务经验，例如目标、约束、踩坑和成功做法。
...
- 默认关闭；开启后使用 `auto_high_confidence` 审批策略，高置信稳定记忆可自动批准，
  `task_episode` 和低置信候选默认保留给用户 Review。
...
1. Flutter UI 与编排层
   ...
   - 每轮 prompt 前调用 memory search，构造独立 memory context block，并通过 Rust
     ACP Core 作为独立 text block 注入 prompt。
...
- 第一版不做 RAG 系统，只做五类经审核 memory。
- `memory-core` 不启动 ACP agent；ACP extractor 由 Flutter 编排独立 sidecar
  session，主 ACP 会话仍由 Rust Core 统一管理。
...
- macOS 构建会把通用架构的 `memory-core` 放入
  `Contents/Resources/memory-core`；主验证脚本同时测试两个 Rust workspace。
