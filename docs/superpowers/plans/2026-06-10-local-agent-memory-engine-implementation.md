...
The original plan predated the Rust-owned ACP production runtime. The completed
mainline integration keeps `memory-core` independent, but carries reviewed
memory context through `RustAcpAgentClient`, the typed Dart FFI host, and
`ianvs-acp-core` as a separate text block before the user prompt. macOS builds
bundle `memory-core` in `Contents/Resources`, and
`tool/verify_rust_runtime.sh` verifies both Rust workspaces.
...
- Modify `lib/acp/rust_acp_agent_client.dart`, `lib/rust/ianvs_acp_native.dart`,
  and Rust Core/FFI: send memory context as a separate ACP text block.
...
- Modify: `lib/acp/rust_acp_agent_client.dart`
- Modify: `lib/rust/ianvs_acp_native.dart`
- Modify: `rust/crates/ianvs-acp-core/src/prompt.rs`
- Modify: `rust/crates/ianvs-acp-core/src/runtime.rs`
- Modify: `rust/crates/ianvs-acp-ffi/src/lib.rs`
...
Modify `RustAcpAgentClient`, the typed Dart FFI host, Rust Core/FFI, and
`FakeAgentClient` to accept `memoryContext`. Rust Core prepends non-empty
reviewed memory as a separate text content block before the user prompt.
`FakeAgentClient` stores `lastMemoryContext = memoryContext` for tests.
...
The client can run a local Rust `memory-core` daemon for five reviewed memory
kinds: user preferences, project rules, architecture decisions, session
summaries, and reusable task episodes. Memory is local-first and disabled by
default. The daemon exposes local HTTP APIs and a stdio MCP bridge; Flutter owns
memory orchestration and sidecar extraction, while Rust Core owns the production
ACP prompt path.
```
