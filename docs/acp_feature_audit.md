...
`session/prompt` using the active workspace, agent, and session scope, while the
Rust `memory-core` daemon owns local storage, review state, search, audit
history, embeddings, and its stdio MCP bridge.
...
| Local Memory Engine | Local extension around ACP | Partial: daemon APIs, callable MCP bridge, and configured prompt search | Flutter carries `MemoryConfig`, Agent Configuration controls for daemon URL/token env, Memory Review/Explorer surfaces, prompt middleware, and sidecar extraction prompts. Memory defaults off; when `daemon_base_url` and a token env are configured, Flutter calls daemon search with active workspace/repo/session scope and injects returned items as background context before `session/prompt`. Daemon search failures/timeouts are non-fatal. Rust `memory-core` owns SQLite/sqlite-vec storage, reviewed CRUD/candidates/change requests/audit, query-aware scope-filtered search/formatting, local and OpenAI-compatible embedding providers, daemon APIs, and MCP stdio `memory.search`/`memory.list`/`memory.remember` calls. `memory-core` never proxies ACP. Full daemon auto-start, live review action wiring, clear-data UX, and real sidecar extraction loop remain follow-ups. |
