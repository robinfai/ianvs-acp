...
than an ACP transport proxy. Flutter injects reviewed memory as background
context before `session/prompt`, while the Rust `memory-core` daemon owns local
storage, review state, search, audit history, embeddings, and its stdio MCP
bridge.
...
| Local Memory Engine | Local extension around ACP | Implemented initial local runtime | Flutter carries `MemoryConfig`, Agent Configuration controls, Memory Review/Explorer surfaces, prompt middleware, and sidecar extraction prompts. Rust `memory-core` owns SQLite/sqlite-vec storage, reviewed CRUD/candidates/change requests/audit, search/formatting, local and OpenAI-compatible embeddings, daemon APIs, and MCP stdio tool listing. Memory is injected as background context before `session/prompt`; `memory-core` never proxies ACP. The first app-level runtime uses a no-op search hook until daemon startup/search/review wiring is connected end to end. |
