...
Schema baseline: `@agentclientprotocol/sdk` 1.2.1. See
[`acp_1_2_1_conformance.md`](acp_1_2_1_conformance.md) for the method-by-method
wire coverage matrix and current Codex App Server verification.
...
| Session delete | https://agentclientprotocol.com/protocol/v1/session-delete | Done | Session settings exposes a separately confirmed destructive `Delete Session` action when `sessionCapabilities.delete` is advertised. The request removes persisted history and clears all local state for that session. |
...
| Prompt turn | https://agentclientprotocol.com/protocol/v1/prompt-turn | Done for text/resource-link/embedded file prompts | `session/prompt`, streaming, stop/cancel, turn-ended status, and user echo suppression are in place. Small text attachments are embedded as `resource.text` when the agent advertises `promptCapabilities.embeddedContext`; image and audio attachments are embedded as `image`/`audio` content when the agent advertises matching prompt capabilities; small generic binary attachments are embedded as `resource.blob` when embedded context is advertised. Prompt-side `@file` and URL mentions are preserved as `resource_link` content even when selected attachments force the raw prompt path, with sentence-ending punctuation kept out of link targets. Unsupported or oversized attachments fall back to `resource_link`. After initialization, the prompt input exposes image and audio picker actions only when those capabilities were advertised, uses media-filtered system pickers, accepts native file drag-and-drop with capability-aware feedback and duplicate suppression, and marks selected files with capability-aware send modes: Image, Audio, Embed, or Link. |
...
| Session config options | https://agentclientprotocol.com/protocol/v1/session-config-options | Done | `session/set_config_option` is supported and config options render in the session settings dialog. `select` and native `boolean` values are supported, `session.configOptions.boolean: {}` is advertised, and grouped select choices retain their group identity while rendering in existing controls. `category` is distinct from choice grouping. `config_option_update` is consumed as complete state, including immediate updates after `session/new`. Initial options returned by new/load/resume/fork are retained. Legacy `models` and modes remain compatibility fallbacks only. |
