...
void main() {
  test('TaskRunnerWorker runs through an exclusive agent lease', () async {
    ...
    final lease = await worker.tryAcquire(task);
    final competingLease = await worker.tryAcquire(task);
    final result = await lease!.run(task);
    await lease.release();
