...
void main() {
  ...
  test(
    'session SSE startup failure is reported once across concurrent sends',
    () async {
      final server = await HttpServer.bind(InternetAddress.loopbackIPv4, 0);
      final openResponses = <HttpResponse>[];
      final inboundMessages = <Map<String, dynamic>>[];
      final transportErrors = <({Object error, StackTrace stackTrace})>[];
      final connectionStreamOpened = Completer<void>();
      final sessionStreamStarted = Completer<void>();
      final releaseSessionFailure = Completer<void>();
      final outboundLines = <String>[];
      ...
      final serverSubscription = server.listen((request) async {
        ...
        if (request.method == 'GET' && sessionId == null) {
          ...
          await request.response.flush();
          openResponses.add(request.response);
          if (!connectionStreamOpened.isCompleted) {
            connectionStreamOpened.complete();
          }
          return;
        }
        if (request.method == 'GET' && sessionId == 'session-1') {
          sessionGetCount += 1;
          if (!sessionStreamStarted.isCompleted) {
            sessionStreamStarted.complete();
          }
          await releaseSessionFailure.future;
          request.response.statusCode = HttpStatus.internalServerError;
          await request.response.close();
          return;
        }
        ...
        final body = await utf8.decoder.bind(request).join();
        final message = jsonDecode(body) as Map<String, dynamic>;
        if (sessionId != null) sessionPostCount += 1;
        request.response.headers.contentType = ContentType.json;
        if (message['method'] == 'initialize') {
          request.response
            ..headers.set('Acp-Connection-Id', 'connection-1')
      ...
      final inboundSubscription = channel.stream.listen(
        ...
        onError: (Object error, StackTrace stackTrace) {
          transportErrors.add((error: error, stackTrace: stackTrace));
        },
      );
      ...
      try {
        ...
        await connectionStreamOpened.future.timeout(const Duration(seconds: 2));

        for (final id in <int>[2, 3]) {
          channel.sink.add(
            jsonEncode(<String, dynamic>{
              'jsonrpc': '2.0',
              'id': id,
        ...
        await sessionStreamStarted.future.timeout(const Duration(seconds: 2));
        await _waitFor(() => outboundLines.length == 3);
        releaseSessionFailure.complete();

        await _waitFor(() => transportErrors.isNotEmpty);
        await pumpEventQueue(times: 4);

        expect(sessionGetCount, 1);
        expect(sessionPostCount, 0);
        expect(transportErrors, hasLength(1));
        final reported = transportErrors.single;
        expect(
          reported.error,
          isA<HttpException>()
              .having(
                (error) => error.message,
                'message',
                'ACP HTTP SSE stream failed with 500: Internal Server Error',
              )
              .having((error) => error.uri, 'uri', transport.endpoint),
        );
        expect(
          reported.stackTrace.toString(),
          contains('_ensureInboundStream'),
        );

        await transport.stop().timeout(const Duration(seconds: 5));
        disposed = true;
      } finally {
        if (!releaseSessionFailure.isCompleted) {
          releaseSessionFailure.complete();
        }
        await inboundSubscription.cancel();
        if (!disposed) await transport.stop();
